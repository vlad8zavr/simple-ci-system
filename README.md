...
7. [Возникшие проблемы (Решено)](#возникшие-проблемы-решено)
...
Для тестирования я использовал свой репозиторий https://github.com/vlad8zavr/verstka-task

(После клонирования установил в нем пакеты `npm i`, затем прописал в конфиге сервера путь до него)
...
3. В случае, если все агенты заняты, а пользователь отправляет новый запрос, сервер отправит на страницу сообщение, что все агенты заняты, и завершит данный запрос. Таким образом приложение устраняет возможность образования очереди запросов и не загружает сеть.

`(/screenshots/ci-app-3.png)`

![page-1](/screenshots/ci-app-3.png)
...
## Возникшие проблемы (Решено)

В ходе выполнения задания я натолкнулся на проблему.
...
Для этого я объект `res` запроса с html на сервер записал в глобальную переменную `RESCLIENT` и в приеме пост запроса с агента по ручке `/notify_build_result` работал с этой копией.
